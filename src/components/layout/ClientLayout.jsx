...
export default function ClientLayout({ children }) {
  ...
  const menuItems = [
    { name: 'Dashboard', icon: 'bi-grid', path: '/client-panel' },
    { name: 'Attorney', icon: 'bi-person', path: '/client-panel/attorneys' },
    { name: 'Case History', icon: 'bi-clock-history', path: '/client-panel/cases' },
    { name: 'Appointments', icon: 'bi-calendar-event', path: '/client-panel/appointments' },
    { name: 'Transaction-Management', icon: 'bi-folder', path: '/client-panel/transaction-management' },
    { name: 'Message', icon: 'bi-chat-left-text', path: '/client-panel/messages' },
    { name: 'Settings', icon: 'bi-gear', path: '/client-panel/settings' },
  ];
  ...
  return (
    <div className="layout-container">
      {/* --- MOBILE NAVBAR --- */}
      <div className="d-lg-none mobile-topbar shadow-sm">
        <button className="btn border-0 text-navy" onClick={() => setShowSidebar(!showSidebar)}>
          <i className={`bi ${showSidebar ? 'bi-x-lg' : 'bi-list'} fs-2`}></i>
        </button>
        <h5 className="m-0 fw-bold text-navy">Lawstick</h5>
        <div className="profile-mini">
           <img src="/assets/images/attorney1.png" className="rounded-circle border" alt="u" />
        </div>
      </div>
      ...
      <div className="container py-lg-5 py-3">
        <div className="row g-4 pt-5">
          ...
          {/* --- SIDEBAR --- */}
          <aside className={`sidebar-wrapper col-lg-3 ${showSidebar ? 'active' : ''}`}>
            <div className="card border-0 shadow-sm rounded-4 overflow-hidden h-100 bg-white">
              <div className="p-4 text-center border-bottom">
                <div className="avatar-container mx-auto mb-3">
                    <img src="/assets/images/attorney1.png"  className="avatar-img shadow-sm" alt="John" />
                </div>
                <h6 className="fw-bold mb-1 text-navy">John</h6>
                <p className="text-muted mb-0 x-small">user@gmail.com</p>
              </div>
          ...
          {/* --- MAIN CONTENT --- */}
          <main className="col-lg-9 content-area">
            {children}
          </main>
        </div>
      </div>
      ...
      <style jsx>{`
        .layout-container { background-color: #f4f7fa; min-height: 100vh; }
        .text-navy { color: #002147; }
        ...
        .profile-mini img { width: 35px; height: 35px; object-fit: cover; }

        .avatar-container { width: 100px; height: 100px; position: relative; }
        .avatar-img { 
          width: 100%; height: 100%; border-radius: 50%; object-fit: cover; 
          border: 4px solid #fcf6ef;
        }

        .sidebar-nav .nav-link { 
          color: #444 !important; font-size: 15px; padding: 12px 18px; 
          border-radius: 12px; transition: 0.3s; margin-bottom: 5px; font-weight: 500; text-decoration: none; display: flex; align-items: center;
        }
        .sidebar-nav .nav-link:hover { background: #f8f9fa; color: #de9f57 !important; }
        .sidebar-nav .nav-link.active { background: #fcf6ef; color: #de9f57 !important; font-weight: bold; }
        ...
        @media (max-width: 991px) {
           .sidebar-wrapper {
              position: fixed; top: 55px; left: -100%; width: 280px; height: calc(100vh - 55px);
              z-index: 1000; transition: 0.3s; padding: 15px; background: #f4f7fa;
           }
