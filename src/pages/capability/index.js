...
function Capabilities() {
  ...
  return (
    <>
      <Head><title>Capabilities & Expertise | Core Law</title></Head>

      {/* HERO */}
      <div className="position-relative d-flex align-items-center justify-content-center" style={{ backgroundImage: 'url(/assets/images/our-firm4.png)', backgroundSize: 'cover', backgroundPosition: 'center', height: '400px' }}>
        <div className="position-absolute w-100 h-100" style={{ backgroundColor: 'rgba(255, 255, 255, 0.4)' }}></div>
        <div className="container position-relative text-center mt-5">
          <h1 className="display-3 fw-bold text-dark font-serif">Capabilities</h1>
          <p className="lead fw-medium text-dark mt-2">Comprehensive legal expertise across industries.</p>
        </div>
      </div>

      {/* SEARCH BAR */}
      <div style={{ backgroundColor: '#cfa144' }} className="py-4 sticky-top">
        <div className="container">
          <div className="row justify-content-center">
            <div className="col-md-6">
              <div className="input-group border-bottom border-dark">
                <span className="input-group-text bg-transparent border-0"><i className="bi bi-search text-dark fs-5"></i></span>
                <input type="text" className="form-control bg-transparent border-0 text-dark rounded-0 fs-5" placeholder="Search Capabilities..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="py-5 bg-white min-vh-100">
        <div className="container">
          <h2 className="font-serif fw-bold text-dark mb-5 pb-3 border-bottom">Expertise A-Z</h2>
          <div className="row">
            <div className="col-lg-6 pe-lg-5">
              {leftColumn.map((item, idx) => (
                <div key={idx} className="mb-3 pb-2 border-bottom border-light">
                   <Link href={`/capability/${createSlug(item.categoryName)}`}>
                      <a className="text-decoration-none"><h5 className="fw-bold text-gold-hover" style={{ color: '#b88b4a' }}>{item.categoryName}</h5></a>
                   </Link>
                </div>
              ))}
            </div>
      ...
      <style jsx>{` .font-serif { font-family: serif; } .text-gold-hover:hover { color: #8f6b36 !important; text-decoration: underline; } `}</style>
    </>
  );
}
export default Capabilities;